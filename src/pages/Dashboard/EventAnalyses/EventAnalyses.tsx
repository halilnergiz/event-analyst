...
export const EventAnalyses = () => {
    ...
    useEffect(() => {
        const checkEvent = async () => {
            try {
                const res = await axios.get(`${process.env.REACT_APP_API_ENDPOINT}/events?userId=${userId}&id=${eventId}`);
                const { data } = res;
                setEventCheck(true);
        ...
        checkEvent();
    }, [eventCheck, eventId, userId]);

    // chart data
    const participantGenders = [
        { id: 0, value: femaleCount, label: 'Kadın Katılımcı' },
        { id: 1, value: maleCount, label: 'Erkek Katılımcı' },
    ];
                        ...
                        <div className="lower-area">
                            <div className="chart age">
                                ...
                                <BarChart
                                    ...
                                    series={[
                                        {
                                            data: eventPersonCheck ? [...participantAges] : [0],
                                        },
                                    ]}
                                    width={600}
                                    height={300}
